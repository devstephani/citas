<div ondragenter="onLivewireCalendarEventDragEnter(event, '{{ $componentId }}', '{{ $day }}', '{{ $dragAndDropClasses }}');"
    ...
    class="flex-1 h-40 lg:h-48 border border-gray-200 -mt-px -ml-px {{ now()->subDays(1)->lt($day) ? ' cursor-pointer' : ' cursor-not-allowed' }}"
    ...
    <div class="w-full h-full" id="{{ $componentId }}-{{ $day }}">
        ...
        <div @if ($dayClickEnabled) wire:click="onDayClick({{ $day->year }}, {{ $day->month }}, {{ $day->day }})" @endif
            class="w-full h-full p-2 {{ $dayInMonth
                ? ($isToday
                    ? 'bg-yellow-100'
                    : (now()->lt($day)
                        ? 'bg-white'
                        : 'bg-neutral-200'))
                : 'bg-neutral-200' }} flex flex-col">
